...
insert into userpermission (user_id,policy_id,access_level) select u.id,p.id,'{{ego_access}}' from egouser u, policy p where u.email = '{{ego_user_email}}' and p.name='{{ego_policy}}'  on conflict do nothing;
